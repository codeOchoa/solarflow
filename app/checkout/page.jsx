    ...
    const makePurchase = async () => {
        if (
            ...
            if (!isValidNameOrLastname(checkoutForm.name)) {
                toast.error("You entered an invalid format for name");
                return;
            }

            if (!isValidNameOrLastname(checkoutForm.lastname)) {
                toast.error("You entered an invalid format for lastname");
                return;
            }

            if (!isValidEmailAddressFormat(checkoutForm.email)) {
                toast.error("You entered an invalid format for email address");
                return;
            }

            if (!isValidNameOrLastname(checkoutForm.cardName)) {
                toast.error("You entered an invalid format for card name");
                return;
            }

            if (!isValidCardNumber(checkoutForm.cardNumber)) {
                toast.error("You entered an invalid format for credit card number");
                return;
            }

            if (!isValidCreditCardExpirationDate(checkoutForm.expirationDate)) {
                toast.error("You entered an invalid format for credit card expiration date");
                return;
            }

            if (!isValidCreditCardCVVOrCVC(checkoutForm.cvc)) {
                toast.error("You entered an invalid format for credit card CVC or CVV");
                return;
            }
